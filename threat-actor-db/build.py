...
def render_brief(actors: list[dict]) -> str:
    """Condensed, readable digest: one entry per actor, no reference tables.

    Intended for reading straight through (including on a phone) rather than
    for lookup. ACTORS.md remains the full reference.
    """
    by_region: dict[str, list[dict]] = defaultdict(list)
    for actor in actors:
        by_region[region_key(actor)].append(actor)
    ...
    add("# Threat Actor Brief — Physical and Hybrid Threats to the Financial Sector")
    add("")
    add(
        f"A condensed digest of all {len(actors)} actors: what each one is, why it matters "
        "to a financial institution, what it has actually done, and where the claims come "
        "from. Generated by `build.py`; the full reference with TTPs, per-variable "
        "confidence and watch indicators is in [`ACTORS.md`](ACTORS.md)."
    )
    add("")
    add(
        "Confidence codes after each entry are `E` existence, `A` attribution, "
        "`F` financial targeting, `S` state tasking — assessed separately, never blended."
    )
    add("")
    add("---")
    add("")
    ...
    for region in REGION_ORDER:
        if region not in by_region:
            continue
        entries = sorted(by_region[region], key=lambda a: a["name"])
        add(f"## {region} ({len(entries)})")
        add("")
        for actor in entries:
            add(render_brief_actor(actor))

    return "\n".join(lines).rstrip() + "\n"


def render_brief_actor(actor: dict) -> str:
    out: list[str] = []
    add = out.append
    ...
    meta = [
        f"`{actor['id']}`",
        humanise(actor["actor_class"]),
        humanise(actor["primary_motivation"]),
        f"{actor['status']}, capability {actor['capability']}",
        f"since {actor.get('active_since', 'unknown')}",
    ]
    add(" · ".join(meta))
    add("")
    add("*" + ", ".join(actor["countries"]) + "*")
    add("")

    if actor.get("aliases"):
        add("**Aka:** " + ", ".join(actor["aliases"]))
        add("")

    add(actor["description"])
    add("")
    add("**Why it matters.** " + actor["financial_sector_relevance"])
    add("")
    ...
    conf = actor["assessments"]
    codes = [
        f"E {conf['existence']['confidence']}",
        f"A {conf['attribution']['confidence']}",
        f"F {conf['financial_targeting']['confidence']}",
    ]
    if "state_tasking" in conf:
        codes.append(f"S {conf['state_tasking']['confidence']}")
    add("**Confidence:** " + " · ".join(codes) + ".")
    add("")
    add(f"> Attribution basis: {conf['attribution']['basis']}")
    add("")

    add("**Sources**")
    add("")
    for source in actor["sources"]:
        add(f"1. [{source['title']}]({source['url']}) — {humanise(source['type'])}")
    add("")
    add("---")
    add("")
...
def main() -> int:
    ...
    BRIEF_OUT.write_text(render_brief(actors), encoding="utf-8")

    for path in (JSON_OUT, MARKDOWN_OUT, BRIEF_OUT):
